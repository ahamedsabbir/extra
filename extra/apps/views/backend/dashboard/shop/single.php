				...
				<div class="col">
				  <div class="card shadow-sm">	
					...
					<div class="text-center" style="margin-top:20px; margin-left:10px; margin-right:10px;" >
					  <img src="<?php echo $product_datatable_value['thumbnail']; ?>" class="rounded img-thumbnail" alt="..." width="100%" style="max-height:500px;">
					</div>
					<div class="card-body">
					  <h3 class="card-text"><?php echo $product_datatable_value["title"]; ?></h3>
					  <span><?php echo $product_datatable_value["description"]; ?></span>
					  <div class="d-flex justify-content-between align-items-center">
						<div class="btn-group">
						  <a href="<?php echo BASE_URL.'post_reach_class/like_submit_function/single/$post_id='.$product_datatable_value['id']; ?>">
							  <button class="btn">
							...
							<?php echo vendor_reach_class::watch_count_function($framework_watch_datatable, $product_datatable_value["id"])+1; ?>
							</button>
						</div>
<?php 
...
?>
					  <a href="<?php echo BASE_URL.'publish_product_datatable_class/update_page_function/'.$product_datatable_value['id']; ?>" class="btn btn-primary">
						<i class="fas fa-edit"></i>						
					  </a>
					  <a href="<?php echo BASE_URL.'publish_product_datatable_class/delete_product_datatable_function/'.$product_datatable_value['id']; ?>" class="btn btn-danger">
						<i class="fas fa-trash-alt"></i>
					  </a>
<?php 
} 
?>
					  <a href="<?php echo BASE_URL.'publish_favorite_class/favorite_function/'.$product_datatable_value['id']; ?>" class="btn btn-primary"><i class="far fa-plus"></i></a>
<?php 
...
if($framework_comment_datatable AND $framework_user_datatable){
	foreach($framework_comment_datatable as $framework_comment_datatable_key => $framework_comment_datatable_value){
			foreach($framework_user_datatable as $framework_user_datatable_key => $framework_user_datatable_value){
				if($framework_comment_datatable_value["user_id"] == $framework_user_datatable_value['id']){
?>
           <div class="py-1">
			<div class="card p-3 py-1 bg-primary"> 
                ...
                <div class="action d-flex justify-content-between mt-2 align-items-center">
                    <div> 
					<a style="color:black;" href="<?php echo BASE_URL.'post_single_class/reply_page_function/reply/&plugins='.$plugins.'&post_id='.$product_datatable_value['id']."&comment_id=".$framework_comment_datatable_value['id']; ?>"><span class="badge bg-danger"><?php echo vendor_reply_class::reply_count($framework_reply_datatable, $framework_comment_datatable_value['id']); ?></span></a>
<?php
