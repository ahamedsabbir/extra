<?php
class dashboard_redweb_insert_class extends backend_controller_class{
	public $plugins;
	public $plugins_type;
		...
		if(isset($_POST["submit"]) AND $_POST["submit"] == "insert"){
			...
			if($post_exist == 0){
				...
				if($this->method_array['text_validation_class']->submit() AND $this->method_array['file_validation_class']->submit()){
					...
					if($insert == true){
						header(address_class::header_page("dashboard_redweb_author_class", "index_page_function", "author"));	
					}else{
			...
			header(address_class::header_page("dashboard_redweb_insert_class", "index_page_function", "insert"));
		}	
	}
