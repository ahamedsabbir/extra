                            ...
                            <div class="card-heading">	
                                <div class="btn-group py-3" role="group" aria-label="Button group with nested dropdown">
                                  <div class="btn-group" role="group">
                                    <h1 id="btnGroupDrop1" type="button" class="btn btn-lg active" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">Subdistrict</h1>
                                  </div>
                                  <button onclick="window.location='<?php echo BACKEND_HOME; ?>'" type="button" class="btn btn-lg">Home</button>
                                  <button type="button" class="btn btn-lg" data-toggle="modal" data-target="#subdistrictModal">view</button>
                                </div>
                            </div>
							 <div class="card-body">
								<form role="form" action="<?php echo BASE_URL.'plugins_location_class/subdistrict_insert_function/subdistrict'; ?>" method="POST" enctype="multipart/form-data">
<div class="form-row">
	<div class="name">District</div>
	<div class="value">
		<div class="input-group">
			<select class="input--style-6" name="district" style="padding:8px;">
              <option>select</option>
<?php
if(isset($framework_district_datatable)){
	foreach($framework_district_datatable as $framework_district_datatable_key => $framework_district_datatable_value){
		$name = $framework_district_datatable_value['bn_name'];
      	$id = $framework_district_datatable_value['id'];
...
?>
									...
									<div class="card-footer">
                                        <button class="btn btn--radius-2 btn--blue-2" type="submit" name="submit" value="save">Save</button>
