...
$backup_config = array(
    ...
    'DB_NAME' => 'mvc_database',//Database name to backup
    ...
    'SAVE_AS' => 'mvc_database_backup',//Prepend filename
...
function backupDB(array $config): string
{
    ...
    if ($do_compress) {
        $save_string = "extra/storage/backup/" . $config['SAVE_AS'] . $config['SAVE_DIR'] . date($config['APPEND_DATE_FORMAT']) . '.sql.gz';
        $zp = gzopen($save_string, "a9");
    } else {
        $save_string = "extra/storage/backup/" . $config['SAVE_AS'] . $config['SAVE_DIR'] . date($config['APPEND_DATE_FORMAT']) . '.sql';
    ...
    return "{$config['DB_NAME']} saved as <a href='$save_string'>Download</a>";
}