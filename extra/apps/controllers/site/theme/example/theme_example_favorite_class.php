<?php
class theme_example_favorite_class extends frontend_example_controller_class{
	public $type = "theme";
	...
	public function page_function($page, $array_key = false, $array_value = false){
  	...
  	public function index_page_function($page, $array_key = false, $array_value = false){	
		...
		$this->model->pagenation("framework_favorite_datatable", array("datatable" => $this->post_datatable, "cookie_id" => cookie_class::id()));
		$this->model->get("example_post_datatable", array("status" => 1));
		$this->view->indexPage($page);
	}
	public function favorite_insert_function($page, $array_key = false, $array_value = false){
		parent::argument_control($array_key, $array_value);
		$this->get_data_array["post_id"] = $post_id = parent::data_control("post_id");
		$insert_datatable = "framework_favorite_datatable";
		...
		if($check == 0){
			...
			if($insert){
				session_class::set("favorite", "add new pot in favorite");
			}else{
				session_class::set("favorite", "not add some problem");
			}
		}
		$msg = vendor_getMsg_class::set("Post ID Not Found......");
		header(address_class::header_page(__CLASS__, "index_page_function", $page, $msg));
	}
	public function favorite_delete_function($page, $array_key = false, $array_value = false){
		parent::argument_control($array_key, $array_value);
		$this->get_data_array["post_id"] = $post_id = parent::data_control("post_id");
		$delete_datatable = "framework_favorite_datatable";
