<?php
class update extends frontend_example_controller_class{
	public $type = "theme";
	public $app = "backend";
	...
	public function page_function($page, $array_key = false, $array_value = false){
  	...
  	public function post_update_function($page, $array_key = false, $array_value = false){
		...
		$update_datatable = isset($_GET['primary_datatable']) ? $_GET['primary_datatable'] : $this->primary_datatable;
		$check_by_array = array("id" => $post_id, "admin_id" => session_class::get('admin_id'));
		$update_by_array = array("id" => $post_id);
		$update_data_array = array();
		$update_data_array["admin_id"] = session_class::get("admin_id");
		$update_data_array["reg"] = 874582365;
