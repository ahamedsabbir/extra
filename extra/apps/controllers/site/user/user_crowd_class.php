<?php
class user_crowd_class extends plugins_controller_class{
	public function __construct(){
		...
		parent::__construct();
		$this->primary_datatable = $this->get_data_array["primary_datatable"] = "framework_user_datatable";
	}
		...
		if(isset($_POST["submit"])){
			...
			if($this->method_array["text_validation_class"]->submit()){
				...
				if($this->get_data_array[$this->primary_datatable] != null){
					cookie_class::set('user_login', true);
					cookie_class::set('user_id', $this->get_data_array[$this->primary_datatable][0]['id']);
                  	cookie_class::set('user_unique_id', $this->get_data_array[$this->primary_datatable][0]['unique_id']);
					cookie_class::set('user_name', $this->get_data_array[$this->primary_datatable][0]['name']);
					cookie_class::set('user_level', $this->get_data_array[$this->primary_datatable][0]['level']);
					cookie_class::set('user_photo', $this->get_data_array[$this->primary_datatable][0]['photo']);
					cookie_class::set('user_survey', $this->get_data_array[$this->primary_datatable][0]['survey']);
					$this->view_load_object_arrays->view_load_functions(backend_class::page($page, $this->plugins_type, $this->plugins), $this->get_data_array);
				}else{
                  	//throw new Exception("user sign error");
	...
	public function user_signup_function($page = null, $array_key = false, $array_value = false){
		if(isset($_POST["submit"]) AND $_POST["submit"] == "sign-up"){
            ...
            $insert_data_array["unique_id"] = "user_".md5(rand(1000000, 9999999));
			$insert_data_array["password"] = md5(parent::data_control("password"));
			$insert_data_array["code"] = md5(rand(1000000, 9999999));
			$insert_data_array["country"] = $this->method_array['visitor_info_class']->country;
			$insert_data_array["city"] = $this->method_array['visitor_info_class']->city;
			$insert_data_array["ip"] = $this->method_array['visitor_info_class']->ip;
			$insert_data_array["browser"] = $this->method_array['visitor_info_class']->browser;
			$insert_data_array["description"] = "empty";
			$insert_data_array["keywords"] = "empty";
			$insert_data_array["photo"] = "empty";
			$insert_data_array["zip_code"] = 8700;
			$insert_data_array["level"] = 3;
			$insert_data_array["survey"] = "new";
