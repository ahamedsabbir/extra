...
class SaveImage extends CommandAbstract
{
    ...
    public function execute(Request $request, WorkingFolder $workingFolder, EventDispatcher $dispatcher, CacheManager $cache, ResizedImageresources $resizedImageresources, Thumbnailresources $thumbnailresources, Acl $acl, Config $config)
    {
        ...
        if (!$editFileEvent->isPropagationStopped()) {
            ...
            if (!$saveAsNew && $saved) {
                ...
                $thumbnailresources->deleteThumbnails($resourceType, $workingFolder->getClientCurrentFolder(), $fileName);
                $resizedImageresources->deleteResizedImages($resourceType, $workingFolder->getClientCurrentFolder(), $fileName);
            }
        }
