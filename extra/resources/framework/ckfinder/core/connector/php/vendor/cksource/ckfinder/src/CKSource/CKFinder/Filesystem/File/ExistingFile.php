...
abstract class ExistingFile extends File
{
    ...
    public function deleteThumbnails()
    {
        ...
        if (Image::isSupportedExtension($extension) || ('bmp' === $extension && $this->config->get('thumbnails.bmpSupported'))) {
            $thumbsresources = $this->resourceType->getThumbnailresources();

            return $thumbsresources->deleteThumbnails($this->resourceType, $this->folder, $this->getFilename());
        }
    ...
    public function deleteResizedImages()
    {
        ...
        if (Image::isSupportedExtension($extension)) {
            $resizedImageresources = $this->resourceType->getResizedImageresources();

            return $resizedImageresources->deleteResizedImages($this->resourceType, $this->folder, $this->getFilename());
        }
